<?php
    ...
    // FIXED: Changed '=' to 'LIKE' to catch PENDING_REPAIR and PENDING_REPLACEMENT
    ...
    // --- TABLES ---
    $result_critical_stock = $conn->query("SELECT product_id, product_name, sku, stock_level, min_threshold FROM products WHERE stock_level <= min_threshold AND stock_level > 0 ORDER BY stock_level ASC LIMIT 5");

    // FIXED: Added status as 'sub_status' and changed '=' to 'LIKE' to capture those two pending items
    $master_log_sql = "
        (SELECT timestamp AS dt, action_type, product_id, user_id, 'Inventory' AS src, '' AS sub_status FROM inventory_log)
        UNION ALL
        (SELECT date_reported AS dt, 'DAMAGE REPORT' AS action_type, product_id, reported_by_user_id AS user_id, 'Damaged' AS src, status AS sub_status FROM damaged_products WHERE status LIKE 'PENDING_%')
        ORDER BY dt DESC LIMIT 6
    ";

    $result_activity_log = $conn->query("
        SELECT combined.*, p.product_name, u.first_name 
        FROM ($master_log_sql) AS combined
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    <?php if ($result_critical_stock->num_rows > 0): ?>
                                        ...
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="4" class="text-center py-4 text-muted">All stock levels are healthy.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
